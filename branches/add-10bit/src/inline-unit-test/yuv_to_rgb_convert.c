...
#define CHECK_FN_8BIT(fn_suffix, input_type)\
    uint32_t    check_ ## fn_suffix() {\
        CHECK_INLINE_1IN(fn_suffix ## _scalar, fn_suffix ## _sse2, input_type, 3, MAX_DIFF_8BIT,  compare_16bit_output);\
        CHECK_INLINE_1IN(fn_suffix ## _scalar, fn_suffix ## _sse2_ssse3, input_type, 3, MAX_DIFF_8BIT,  compare_16bit_output);\
        return 0;\
    }

#define CHECK_FN_10BIT(fn_suffix, input_type_8bit, input_type_10bit)\
    uint32_t    check_ ## fn_suffix() {\
        CHECK_INLINE_1IN(fn_suffix ## _scalar, fn_suffix ## _sse2, input_type_8bit, 3, MAX_DIFF_8BIT,  compare_16bit_output);\
        CHECK_INLINE_1IN(fn_suffix ## _scalar, fn_suffix ## _sse2, input_type_10bit, 3, MAX_DIFF_8BIT,  compare_16bit_output);\
        CHECK_INLINE_1IN(fn_suffix ## _scalar, fn_suffix ## _sse2_ssse3, input_type_8bit, 3, MAX_DIFF_8BIT,  compare_16bit_output);\
        CHECK_INLINE_1IN(fn_suffix ## _scalar, fn_suffix ## _sse2_ssse3, input_type_10bit, 3, MAX_DIFF_8BIT,  compare_16bit_output);\
        return 0;\
    }

#define DECLARE_NNB_UPSAMPLE_N_CONVERT_CHECK(fn_prefix, coeffs, offset, check_fn) \
 ...
 */
void nnb_upsample_n_convert_y_uv_vectors_to_rgb_vectors_scalar(__m128i* in_2_v16i_y_uv_vectors, __m128i* out_3_v16i_rgb_vectors) {
